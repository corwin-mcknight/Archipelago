...
## Memory
The vendor U-Boot clamps `ram_top` to 4 GiB and reports the DRAM above it as EFI boot-services data: never offered to EFI allocations, but never reported free either. Limine passes that through as bootloader-reclaimable memory, which the kernel otherwise leaves alone because Limine's own data lives in that class. The board reports the 4 GiB line through `kernel::platform::firmware_fenced_memory_base()`, and the Limine boot code hands any reclaimable memory above it to the page pool, so the 4 GiB board sees its full DRAM (the top gigabyte at 0x100000000). The firmware DTB is no help here: its `/memory` node describes 2 GiB regardless of the fitted DRAM.
