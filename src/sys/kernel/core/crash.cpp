...
const char* trigger_name(trigger_kind kind) {
    switch (kind) {
        case trigger_kind::panic: return "panic";
        case trigger_kind::exception: return "exception";
        case trigger_kind::assertion: return "assertion";
...
void emit_header_harness(trigger_kind kind, register_frame_t* regs, const char* message, const char* file, int line) {
    uint64_t vec  = regs ? arch::frame_vec(regs) : 0;
    uint64_t err  = regs ? arch::frame_err(regs) : 0;
    uint64_t time = static_cast<uint64_t>(kernel::time::ns_since_boot());

    crash_write("\n@@CRASH_BEGIN {\"trigger\":\"");
    crash_write(trigger_name(kind));
    crash_write("\"");
...
void emit_header_prose(trigger_kind kind, register_frame_t* regs, const char* message, const char* file, int line) {
    crash_write("\n*** KERNEL CRASH ***\n");
    crash_emit("Trigger: {0}", trigger_name(kind));
    if (regs) {
        crash_emit(" (vec={0} {1}, err=0x{2:016p})\n", arch::frame_vec(regs),
                   arch::exception_name(static_cast<uint32_t>(arch::frame_vec(regs))), arch::frame_err(regs));
    } else {
        crash_write("\n");
    }
...
void emit_stack(register_frame_t* regs, bool harness) {
    if (!regs) return;
    uintptr_t rsp = arch::frame_sp(regs);
...
[[noreturn]] void dispatch(trigger_kind kind, register_frame_t* regs, const char* message, const char* file, int line) {
    ...
    if (harness) {
        ...
        if (regs) { arch::emit_registers_harness(regs, cr); }
        ...
        if (regs) {
            arch::emit_registers_prose(regs, cr);
        } else {
