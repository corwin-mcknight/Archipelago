...
struct cpu_core {
    uintptr_t kstack_top   = 0;  // stack the next trap from user mode lands on
    uintptr_t user_sp      = 0;  // parking slot for the interrupted user sp during trap entry
    uintptr_t kstack_floor = 0;  // stack-overflow tripwire for the thread running on this hart
    size_t index           = 0;  // dense bootloader CPU-list position; the subscript into g_cpu_cores
    uint64_t hartid        = 0;
