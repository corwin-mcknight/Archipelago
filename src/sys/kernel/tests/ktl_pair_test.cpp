...
KTEST(ktl_pair_construct_and_access, "ktl/pair") {
...
KTEST(ktl_pair_make_pair_decays, "ktl/pair") {
    auto p = ktl::make_pair(1, 2u);
    KTEST_EXPECT_TRUE(p.first == 1);
    KTEST_EXPECT_TRUE(p.second == 2u);
    static_assert(ktl::is_same<decltype(p)::second_type, unsigned int>::value);
}

KTEST(ktl_pair_equality, "ktl/pair") {
...
KTEST(ktl_pair_move_and_swap, "ktl/pair") {
