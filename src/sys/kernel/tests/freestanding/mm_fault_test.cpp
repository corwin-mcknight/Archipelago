...
// Demand-paging tests: real #PF faults resolved through the VMM. The four
// scenarios -- zero-page read, CoW write break, direct write fill, and
// resolution in a non-kernel aspace -- form one integration story against a
...
KTEST_CASE_INTEGRATION(fault_demand_paging_story) {
    ...
    // Phase 2: a write through the shared zero page breaks CoW into a private
    // copy owned by the VMO.
    {
        auto v = create_anonymous_vmo(PAGES);
        KTEST_REQUIRE_TRUE(v.get() != nullptr);
        KTEST_REQUIRE_TRUE(kernel_aspace().root().map(MAP_BASE, PAGES * PAGE, v, 0, RW).is_ok());
        ...
        *word = 0x5EED'F00D'CAFE'D00Dull;
        ...
        KTEST_EXPECT_EQUAL(*word, 0x5EED'F00D'CAFE'D00Dull);
        // A stale TLB entry would satisfy the read without faulting; the
        // increment proves unmap_page flushed the translation. QEMU's TCG TLB
        // evicts aggressively, so only real hardware reliably fails this on a
        // missing sfence.vma.
    ...
    // Phase 3: writing an unread page fills a private frame directly, never
    // touching the zero page.
    {
        auto v = create_anonymous_vmo(PAGES);
        KTEST_REQUIRE_TRUE(v.get() != nullptr);
        KTEST_REQUIRE_TRUE(kernel_aspace().root().map(MAP_BASE, PAGES * PAGE, v, 0, RW).is_ok());
    ...
    // Phase 4: demand paging works in a non-kernel aspace: the handler
    // resolves against whichever space is active, using its own region tree.
    {
        vm_aspace scratch;
